...
contextBridge.exposeInMainWorld("alivoSystem", Object.freeze({
  ...
  onIntegrationChanged: (listener) => { if (typeof listener !== "function") return () => {}; const handler = () => listener(); ipcRenderer.on("integration:changed", handler); return () => ipcRenderer.removeListener("integration:changed", handler); },
}));
...
contextBridge.exposeInMainWorld("alivoPinterest", Object.freeze({
  ...
  publisherCapabilities: () => ipcRenderer.invoke("pinterest:publisher-capabilities"),
  publishTestPin: (request) => ipcRenderer.invoke("pinterest:publish-test", request),
  // Intentionally exposed for architecture verification; main process hard-locks this path in RUNTIME-005D.
  publishProductionPin: (request) => ipcRenderer.invoke("pinterest:publish-production", request),
