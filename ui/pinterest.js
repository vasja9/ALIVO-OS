...
function topPinsView(){
  ...
  if(rows.length&&result?.window){const immutableRows=Object.freeze(rows.slice());card.append(createElement("h3","","Observed rates"),createElement("p","quiet","Descriptive event ratios for the selected 30-day window. They are not predictions, conversion attribution, or causal analysis."),topPinsComparison(immutableRows,result.window),topPinsTable(immutableRows))}
  return card;
}
