...
let topPinsTableSession = { result: null, filter: "all", sort: { key: "rank", direction: "ascending" }, expandedRank: null };
let pollAttempts = 0;
...
function syncTopPinsTableSession(result){
  ...
  topPinsTableSession=retainedStale?{...topPinsTableSession,result,expandedRank}:{result,filter:"all",sort:{key:"rank",direction:"ascending"},expandedRank:null};
}
...
function topPinsTable(sourceRows,account,topPins){
  ...
  const renderTable=()=>{
    const selected=filters.find(filter=>filter.key===topPinsTableSession.filter)??filters[0],column=topPinsColumns.find(item=>item.key===topPinsTableSession.sort.key)??topPinsColumns[0],filtered=rows.filter(selected.matches),renderedRows=stableTopPinsRows(filtered.slice(),column,topPinsTableSession.sort.direction);
    for(const button of filterButtons)button.setAttribute("aria-pressed",String(button.dataset.topPinsFilter===selected.key));
    resultCount.textContent=`Showing ${renderedRows.length} of ${rows.length} snapshot-matched Top Pins.`;empty.hidden=renderedRows.length!==0;
