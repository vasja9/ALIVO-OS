...
let topPinsTableSession = { result: null, filter: "all", sort: { key: "rank", direction: "ascending" } };
let pollAttempts = 0;
...
function topPinsTable(sourceRows,account,topPins){
  ...
  ].map(filter=>Object.freeze({...filter,count:rows.filter(filter.matches).length}));
  const controls=createElement("div","top-pins-filter-control"),label=createElement("span","top-pins-filter-label","Show Top Pins:"),buttons=createElement("div","top-pins-filter-buttons"),filterButtons=[],resultCount=createElement("p","top-pins-filter-result"),empty=createElement("p","top-pins-filter-empty","No Top Pins match this local filter.");
  controls.setAttribute("role","group");controls.setAttribute("aria-label","Show Top Pins");resultCount.setAttribute("aria-live","polite");empty.setAttribute("aria-live","polite");
  const renderTable=()=>{const selected=filters.find(filter=>filter.key===topPinsTableSession.filter)??filters[0],column=topPinsColumns.find(item=>item.key===topPinsTableSession.sort.key)??topPinsColumns[0],filtered=rows.filter(selected.matches),renderedRows=stableTopPinsRows(filtered.slice(),column,topPinsTableSession.sort.direction);for(const button of filterButtons)button.setAttribute("aria-pressed",String(button.dataset.topPinsFilter===selected.key));resultCount.textContent=`Showing ${renderedRows.length} of ${rows.length} snapshot-matched Top Pins.`;empty.hidden=renderedRows.length!==0;const header=createElement("tr");for(const item of topPinsColumns){const cell=createElement("th"),button=createElement("button","top-pins-sort"),indicator=createElement("span","top-pins-sort-indicator");cell.setAttribute("scope","col");const active=topPinsTableSession.sort.key===item.key;cell.setAttribute("aria-sort",active?topPinsTableSession.sort.direction:"none");button.type="button";button.dataset.topPinsSort=item.key;button.append(createElement("span","",item.label),indicator);indicator.setAttribute("aria-hidden","true");if(active)indicator.textContent=topPinsTableSession.sort.direction==="ascending"?"▲":"▼";button.onclick=()=>{const direction=active?(topPinsTableSession.sort.direction==="ascending"?"descending":"ascending"):(item.type==="text"?"ascending":"descending");topPinsTableSession={...topPinsTableSession,sort:{key:item.key,direction}};renderTable()};cell.append(button);header.append(cell)}const rendered=renderedRows.map(item=>{const row=createElement("tr"),pinCell=createElement("td"),statusLine=createElement("div","top-pins-readiness-line");statusLine.append(createElement("span",`state top-pins-readiness-status ${topPinsReadinessClass(item.contentReadiness)}`,topPinsReadinessText(item.contentReadiness)));pinCell.append(createElement("div","top-pins-title",item.title),statusLine);row.append(createElement("td","",item.rank),pinCell,createElement("td","",item.boardName));for(const key of ["impressions","saves","pinClicks","outboundClicks"])row.append(createElement("td","",dailyMetricValue(item[key])));for(const key of ["saveRate","pinClickRate","outboundClickRate"])row.append(createElement("td","",observedRateValue(item[key])));row.append(createElement("td","",topPinsOutboundDifferenceValue(item.outboundVsAccount)));return row});head.replaceChildren(header);body.replaceChildren(...rendered)};
  for(const filter of filters){const button=createElement("button","top-pins-filter",`${filter.label} (${filter.count})`);button.type="button";button.dataset.topPinsFilter=filter.key;button.onclick=()=>{topPinsTableSession={...topPinsTableSession,filter:filter.key};renderTable()};filterButtons.push(button);buttons.append(button)}controls.append(label,buttons);renderTable();table.append(head,body);wrapper.append(table);section.append(summary,controls,createElement("p","top-pins-filter-copy","Local table filters use only the current snapshot. They do not score or recommend Pins."),resultCount,empty,wrapper);return section;
}
...
function topPinsView(){
  ...
  const rows=[];
  for(const pin of (result?.pins??[]).slice(0,25))rows.push(Object.freeze({rank:rows.length+1,title:pin.title,boardName:pin.boardName,impressions:pin.impressions,saves:pin.saves,pinClicks:pin.pinClicks,outboundClicks:pin.outboundClicks,saveRate:observedRate(pin.saves,pin.impressions),pinClickRate:observedRate(pin.pinClicks,pin.impressions),outboundClickRate:observedRate(pin.outboundClicks,pin.impressions),contentReadiness:pin.contentReadiness}));
