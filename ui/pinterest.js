...
const topPinsLifecycleCohorts=Object.freeze([Object.freeze({key:"Days0To59",label:"0–59 days"}),Object.freeze({key:"Days60To90",label:"60–90 days"}),Object.freeze({key:"Days91To180",label:"91–180 days"}),Object.freeze({key:"Days181To600",label:"181–600 days"}),Object.freeze({key:"Days601Plus",label:"601+ days"}),Object.freeze({key:"Unknown",label:"Unknown age"})]);
...
function attentionView() {
  ...
  fragment.append(topPinsReviewWorkspace());
  return fragment;
}
...
function topPinsSafeRows(result){
  const rows=[];
  for(const pin of (Array.isArray(result?.pins)?result.pins:[]).slice(0,25)){
    if(!pin||typeof pin!=="object")continue;
    const readiness=pin.contentReadiness&&["Ready","NeedsAttention"].includes(pin.contentReadiness.status)&&[pin.contentReadiness.issueCount,pin.contentReadiness.requiredIssueCount,pin.contentReadiness.reviewIssueCount].every(value=>Number.isSafeInteger(value)&&value>=0)&&pin.contentReadiness.issueCount===pin.contentReadiness.requiredIssueCount+pin.contentReadiness.reviewIssueCount&&pin.contentReadiness.issueCount<=12?Object.freeze({status:pin.contentReadiness.status,issueCount:pin.contentReadiness.issueCount,requiredIssueCount:pin.contentReadiness.requiredIssueCount,reviewIssueCount:pin.contentReadiness.reviewIssueCount}):null;
    const required=Array.isArray(pin.contentReadinessDetails?.required)?pin.contentReadinessDetails.required.slice(0,12):null,review=Array.isArray(pin.contentReadinessDetails?.review)?pin.contentReadinessDetails.review.slice(0,12):null,details=readiness&&required&&review&&required.length+review.length<=12&&required.length===readiness.requiredIssueCount&&review.length===readiness.reviewIssueCount&&[...required,...review].every(message=>typeof message==="string")?Object.freeze({required:Object.freeze(required.slice()),review:Object.freeze(review.slice())}):null;
    const lifecycleCohort=topPinsLifecycleCohorts.some(item=>item.key===pin.lifecycle?.cohort)?pin.lifecycle.cohort:"Unknown",outboundState=["ReachedAlivoEu","NoOutboundClickInWindow","Unavailable"].includes(pin.lifecycle?.outboundState)?pin.lifecycle.outboundState:"Unavailable",lifecycle=Object.freeze({cohort:lifecycleCohort,outboundState});
    const impressions=comparisonInteger(pin.impressions),saves=comparisonInteger(pin.saves),pinClicks=comparisonInteger(pin.pinClicks),outboundClicks=comparisonInteger(pin.outboundClicks);
    rows.push(Object.freeze({rank:rows.length+1,title:pin.title,boardName:pin.boardName,impressions,saves,pinClicks,outboundClicks,saveRate:observedRate(saves,impressions),pinClickRate:observedRate(pinClicks,impressions),outboundClickRate:observedRate(outboundClicks,impressions),lifecycle,contentReadiness:readiness,contentReadinessDetails:details}));
  }
  return Object.freeze(rows.slice());
}

function topPinsLifecycleView(sourceRows,stale=false){
  const rows=sourceRows.slice(0,25),section=createElement("section","top-pins-lifecycle"),wrapper=createElement("div","top-pins-lifecycle-table-wrap"),table=createElement("table","top-pins-lifecycle-table"),head=createElement("thead"),headingRow=createElement("tr"),body=createElement("tbody");
  section.append(createElement("h3","","Observed Pin lifecycle"),createElement("p","top-pins-lifecycle-copy","Each age cohort uses the same current 30 completed UTC days. This is descriptive evidence for snapshot-matched Top Pins, not the full published Pin population."),createElement("p","top-pins-lifecycle-zero","No outbound click yet means zero outbound clicks in this observed window. It does not mean the Pin failed."),createElement("p","top-pins-lifecycle-history","Creative variant eligibility is not determined here. A Pin aged 60 days or more requires complete since-publication click history, which this session snapshot does not contain."));
  if(stale)section.append(createElement("p","top-pins-lifecycle-stale","Lifecycle comparison uses retained Top Pins data and may be stale."));
  for(const label of ["Pin age","Pins","Reached alivo.eu","No outbound click yet","Unavailable"]){const cell=createElement("th","",label);cell.setAttribute("scope","col");headingRow.append(cell)}head.append(headingRow);
  for(const cohort of topPinsLifecycleCohorts){const group=rows.filter(item=>item.lifecycle.cohort===cohort.key),reached=group.filter(item=>item.lifecycle.outboundState==="ReachedAlivoEu").length,noClick=group.filter(item=>item.lifecycle.outboundState==="NoOutboundClickInWindow").length,unavailable=group.length-reached-noClick,row=createElement("tr");row.append(createElement("th","",cohort.label),createElement("td","",group.length),createElement("td","",reached),createElement("td","",noClick),createElement("td","",unavailable));row.children[0].setAttribute("scope","row");body.append(row)}
  table.append(head,body);wrapper.append(table);section.append(wrapper);return section;
}
...
function topPinsReviewWorkspaceDetails(details){
  const container=createElement("div","top-pins-review-workspace-details");
  if(!details){container.append(createElement("p","top-pins-review-workspace-details-unavailable","Readiness details unavailable."));return container}
  const required=details.required.slice(0,12),review=details.review.slice(0,12-required.length),requiredGroup=createElement("section","top-pins-review-detail-group required"),reviewGroup=createElement("section","top-pins-review-detail-group review");
  requiredGroup.append(createElement("h4","","Required readiness details"));if(required.length){for(const message of required)requiredGroup.append(createElement("p","",message))}else requiredGroup.append(createElement("p","","None."));
  reviewGroup.append(createElement("h4","","Review readiness details"));if(review.length){for(const message of review)reviewGroup.append(createElement("p","",message))}else reviewGroup.append(createElement("p","","None."));
