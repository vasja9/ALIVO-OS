...
if(integration==='wordpress'){
  ...
  form.innerHTML=`<p class="quiet">Connect ALIVO OS with Pinterest OAuth so access can be refreshed automatically.</p>${input('appId','Pinterest App ID')}${input('appSecret','Pinterest App secret','password')}<label>Redirect URI<input id="redirectUri" type="text" readonly value="Loading…"></label><p class="quiet">Add this exact Redirect URI to the Pinterest Developer app before connecting.</p><details><summary>Temporary 24-hour test token</summary>${input('accessToken','Pinterest access token','password','pina… / product-limited token')}<button id="temporaryVerify" type="button" class="secondary">Verify temporary token</button></details>`;
...
verify.onclick=async()=>{
  ...
  if(integration==='pinterest'){
    ...
    try{
      const result=await window.alivoAuth.pinterestOAuth({appId:document.querySelector('#appId').value,appSecret:document.querySelector('#appSecret').value});
      if(await showResult(result))return;
    }catch{status.textContent='Pinterest OAuth could not be completed.';status.className='status error'}
