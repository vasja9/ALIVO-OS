...
interface PinterestRequest { readonly baseUrl:string; readonly environment:PinterestEnvironment; readonly path:string; readonly query:Readonly<Record<string,string>>; readonly timeoutMs:number; readonly session:AuthenticationSessionReference; }
...
export class PinterestMarketSourceAdapter implements MarketSourceAdapter {
  ...
  async collect(request:MarketCollectionRequest):Promise<MarketCollectionResult>{
    ...
    for(let page=0;page<this.configuration.maximumPages;page++){
      ...
      try{response=await this.transport.execute({baseUrl:this.configuration.apiBaseUrl,environment:this.configuration.environment,...mapped,query:{...mapped.query,...(continuation?{bookmark:continuation}:{})},timeoutMs:this.configuration.timeoutMs,session:auth.session});}
  ...
  async normalize(raw:RawMarketRecord):Promise<MarketNormalizationResult>{
    ...
    const canonical={resourceId:p.id,resourceType:p.type,ownership:p.ownership,dataState:PinterestValueState.Observed,createdAt:p.type==="pin"?safeDate(p.observedAt)?.toISOString():undefined,title:p.title,description:p.description,link:p.link,boardReference:p.boardId,media:p.media,keyword:p.keyword,rank:p.rank,region:p.region,growth:p.growth,analytics:p.type==="analytics"?{metric:p.metric,value:p.value,windowStart:p.windowStart,windowEnd:p.windowEnd}:undefined,competitorPrivateMetrics:{clicks:PinterestValueState.Unavailable,ctr:PinterestValueState.Unavailable,conversions:PinterestValueState.Unavailable,revenue:PinterestValueState.Unavailable}};
  ...
  private mapRequest(request:MarketCollectionRequest):Omit<PinterestRequest,"baseUrl"|"environment"|"timeoutMs"|"session">{const capability=request.properties.capability.value;const path=capability===MarketSourceCapability.TrendObservation.value?`/v5/trends/keywords/${encodeURIComponent(request.properties.marketContext)}/top/growing`:capability===MarketSourceCapability.PerformanceObservation.value||capability===MarketSourceCapability.AnalyticsObservation.value?"/v5/user_account/analytics":capability===MarketSourceCapability.MarketObservation.value&&request.properties.subjectReference?.startsWith("board:")?`/v5/boards/${encodeURIComponent(request.properties.subjectReference.slice(6))}`:"/v5/pins";const query:Record<string,string>={page_size:String(this.configuration.pageSize)};if(path==="/v5/pins")return {path,query};query.market=request.properties.marketContext;if(request.properties.languageContext)query.language=request.properties.languageContext;if(request.properties.subjectReference)query.subject=request.properties.subjectReference;if(request.properties.windowStart)query.start_date=request.properties.windowStart.toISOString();if(request.properties.windowEnd)query.end_date=request.properties.windowEnd.toISOString();return {path,query};}
  private parsePage(body:unknown,path:string):{records:ProviderRecord[];continuation?:string}|undefined{const root=object(body);if(!root||!Array.isArray(root.items)||(root.bookmark!==undefined&&root.bookmark!==null&&typeof root.bookmark!=="string"))return undefined;const records:ProviderRecord[]=[];for(const item of root.items){const v=object(item);if(!v||typeof v.id!=="string"||!v.id.trim())return undefined;if(path==="/v5/pins"){const record:ProviderRecord={id:v.id,type:"pin",ownership:v.is_owner===true?PinterestOwnership.OwnedAuthorizedResource:PinterestOwnership.UnknownOwnership};if(typeof v.created_at==="string")record.observedAt=v.created_at;for(const [providerKey,recordKey] of [["title","title"],["description","description"],["link","link"],["board_id","boardId"]] as const)if(typeof v[providerKey]==="string")record[recordKey]=v[providerKey] as never;if(v.media!==undefined)record.media=v.media;records.push(record);continue;}if(!(["pin","board","trend","analytics"] as unknown[]).includes(v.type))return undefined;const ownership=Object.values(PinterestOwnership).includes(v.ownership as PinterestOwnership)?v.ownership as PinterestOwnership:PinterestOwnership.UnknownOwnership;const record:ProviderRecord={id:v.id,type:v.type as ProviderRecord["type"],ownership};for(const key of ["observedAt","title","description","link","boardId","keyword","region","metric","windowStart","windowEnd"] as const)if(typeof v[key]==="string")record[key]=v[key] as never;for(const key of ["rank","growth","value"] as const)if(typeof v[key]==="number"&&Number.isFinite(v[key]))record[key]=v[key] as never;if(v.media!==undefined)record.media=v.media;records.push(record);}return {records,...(typeof root.bookmark==="string"&&root.bookmark?{continuation:root.bookmark}:{})};}
