...
test("observed rates enforce exact local formulas, unrounded sorting, uncapped values, and safe missing states",async()=>{
  ...
  assert.deepEqual(rows().map(row=>row.children.slice(7,10).map(cell=>cell.textContent)),[["33.33%","0.00%","—"],["33.33%","0.00%","120.00%"],["—","—","—"],["—","—","—"]]);saveSort.pressKey("Enter");assert.deepEqual(rows().slice(0,2).map(row=>row.children[1].textContent),["Rate 2","Rate 1"]);assert.equal(JSON.stringify(values),original);assert.equal(harness.callCount("readTopPins"),1);
