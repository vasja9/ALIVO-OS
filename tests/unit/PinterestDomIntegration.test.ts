...
function preloadFor({
  ...
  const observation=async(...args)=>{const result=await readObservation(...args);if(Array.isArray(result?.pins)&&(!["Failed","Unavailable"].includes(result.state)||result.pins.length))retainedSnapshot=result.pins.slice(0,25);return result};
  const topPins=async(...args)=>{const result=await readTopPins(...args);if(!Array.isArray(result?.pins))return result;const byId=new Map(retainedSnapshot.map(pin=>[pin.pinId,pin]));return{...result,pins:result.pins.slice(0,25).flatMap(item=>{const snapshotPin=typeof item?.title==="string"?item:byId.get(item?.pinId);if(!snapshotPin)return[];return[{...item,title:snapshotPin.title||"Untitled Pin",boardName:snapshotPin.boardName||"Unknown board",contentReadiness:item.contentReadiness??null}]})}};
  return { startOAuth, connectionStatus, verifyConnection, readObservation:observation, readAccountPerformance, readTopPins:topPins, readPerformance };
}
...
test("Top Pins comparison and table sorting are local, stable, accessible, immutable, and raw-ID safe",async()=>{
  ...
  assert.deepEqual(rows().map(row=>row.children[1].children[0].textContent),["Zulu","alpha","Alpha","Missing"]);assert.deepEqual(rows().map(row=>row.children.slice(7,10).map(cell=>cell.textContent)),[["—","—","—"],["40.00%","0.00%","80.00%"],["20.00%","40.00%","160.00%"],["—","—","—"]]);assert.deepEqual(headers().map(cell=>cell.getAttribute("aria-sort")),["ascending",...Array(10).fill("none")]);assert.equal(headers()[0].textContent,"Rank▲");assert.equal(harness.hasText("Observed rates"),true);assert.equal(harness.hasText("Descriptive event ratios for the selected 30-day window. They are not predictions, conversion attribution, or causal analysis."),true);const before=requestCounts();
  for(const selector of selectors()){selector.pressKey(selector===selectors()[0]?"Enter":" ");assert.equal(selectors().filter(item=>item.getAttribute("aria-pressed")==="true").length,1)}assert.deepEqual(requestCounts(),before);assert.deepEqual(rows().map(row=>row.children[1].children[0].textContent),["Zulu","alpha","Alpha","Missing"]);const selectMetric=key=>harness.document.querySelector(`[data-top-pins-metric="${key}"]`);selectMetric("saveRate").click();assert.deepEqual(valueLabels().map(label=>label.textContent),["40.00%","20.00%","—","—"]);assert.deepEqual(rankLabels().map(label=>label.textContent),["#2","#3","#1","#4"]);selectMetric("pinClickRate").click();assert.deepEqual(valueLabels().map(label=>label.textContent),["40.00%","0.00%","—","—"]);selectMetric("outboundClickRate").click();assert.deepEqual(requestCounts(),before);
  assert.equal(chart().getAttribute("aria-label"),"Outbound click rate Top Pins comparison from 24.07.26 to 22.08.26 with 2 usable values in percent.");assert.equal(harness.document.querySelector(".top-pins-y-label").textContent,"Outbound click rate (%)");assert.deepEqual(harness.document.querySelectorAll(".top-pins-tick-label").map(label=>label.textContent),["0.00%","50.00%","100.00%","150.00%","200.00%"]);assert.deepEqual(valueLabels().map(label=>label.textContent),["160.00%","80.00%","—","—"]);assert.deepEqual(rankLabels().map(label=>label.textContent),["#3","#2","#1","#4"]);const chartOrder=bars().map(bar=>bar.getAttribute("aria-label"));for(const [key,firstDirection] of [["rank","descending"],["title","ascending"],["boardName","ascending"],["impressions","descending"],["saves","descending"],["pinClicks","descending"],["outboundClicks","descending"],["saveRate","descending"],["pinClickRate","descending"],["outboundClickRate","descending"],["outboundVsAccount","descending"]]){button(key).pressKey(key==="rank"?"Enter":" ");const active=headers().find(cell=>cell.children[0].dataset.topPinsSort===key);assert.equal(active.getAttribute("aria-sort"),firstDirection);assert.equal(/[▲▼]$/.test(active.textContent),true);assert.equal(headers().filter(cell=>cell.getAttribute("aria-sort")!=="none").length,1);if(!["rank","title","boardName"].includes(key))assert.equal(rows().at(-1).children[1].children[0].textContent,"Missing");button(key).click();if(!["rank","title","boardName"].includes(key))assert.equal(rows().at(-1).children[1].children[0].textContent,"Missing")}
...
test("Top Pins readiness context is snapshot-bound, reconciled, sort-stable, text-safe, and request-isolated",async()=>{
  const window={startDate:"2026-07-24",endDate:"2026-08-22",completedDays:30},pins=[{pinId:"readiness-private-1",title:"<img src=x onerror=private()>",boardName:"Board A",thumbnail:null},{pinId:"readiness-private-2",title:"Bravo",boardName:"Board B",thumbnail:null},{pinId:"readiness-private-3",title:"Charlie",boardName:"Board C",thumbnail:null}],audit={state:"Available",analyzedPins:3,readyPins:1,attentionPins:2,issueCounts:{},pins:[{pinId:"readiness-private-1",status:"Ready",issues:[]},{pinId:"readiness-private-2",status:"NeedsAttention",issues:[{code:"TITLE_MISSING",level:"Required",message:"private audit message"},{code:"DESCRIPTION_MISSING",level:"Review",message:"private review message"}]},{pinId:"readiness-private-3",status:"NeedsAttention",issues:[{code:"THUMBNAIL_MISSING",level:"Review",message:"private unavailable source"}]}]},observation={ok:true,state:"Completed",pins,audit},top={ok:true,state:"Available",window,sortBy:"OUTBOUND_CLICK",pins:[{title:pins[0].title,boardName:pins[0].boardName,impressions:10,saves:1,pinClicks:2,outboundClicks:3,contentReadiness:{status:"Ready",issueCount:0,requiredIssueCount:0,reviewIssueCount:0},pinId:"must-discard",issueCodes:["TITLE_MISSING"],providerPayload:"private-provider"},{title:"Bravo",boardName:"Board B",impressions:30,saves:2,pinClicks:3,outboundClicks:4,contentReadiness:{status:"NeedsAttention",issueCount:2,requiredIssueCount:1,reviewIssueCount:1},rawAudit:{message:"private"}},{title:"Charlie",boardName:"Board C",impressions:20,saves:0,pinClicks:1,outboundClicks:0,contentReadiness:null,issueMessage:"private"}],stale:false},account={ok:true,state:"Available",window,latestAvailableDate:window.endDate,totals:{impressions:100,saves:10,pinClicks:20,outboundClicks:10},daily:[],stale:false};
  const harness=await createPinterestDomHarness(preloadFor({connectionStatus:async()=>authenticated,readObservation:sequence(observation,{...observation,state:"CompletedWithWarnings"},{ok:true,state:"Failed"},{...observation,pins:pins.map((pin,index)=>index===0?{...pin,title:"Changed title"}:pin)}),readAccountPerformance:async()=>account,readTopPins:async()=>top,readPerformance:async()=>({ok:true,state:"Available",window,totals:account.totals,pins:[],stale:false})})).start(),counts=()=>["readObservation","readTopPins","readAccountPerformance","readPerformance"].map(name=>harness.callCount(name));
  assert.deepEqual(counts(),[0,0,0,0]);await harness.clickAction("verify");assert.deepEqual(counts(),[0,0,0,0]);await harness.clickAction("observe");assert.deepEqual(counts(),[1,0,0,0]);const globalAudit=harness.document.querySelector(".content-readiness").textContent;harness.document.querySelector('[data-pin-view="performance"]').click();await harness.settle();assert.deepEqual(counts(),[1,0,0,0]);await harness.clickAction("account-performance");assert.deepEqual(counts(),[1,0,1,0]);assert.equal(harness.document.querySelector(".top-pins-readiness-summary"),undefined);await harness.clickAction("top-pins");assert.deepEqual(counts(),[1,1,1,0]);
  const table=()=>harness.document.querySelector(".top-pins-table"),rows=()=>table().children[1].children,pairs=()=>rows().map(row=>[row.children[1].children[0].textContent,row.children[1].children[1].textContent]),summary=()=>harness.document.querySelector(".top-pins-readiness-summary");
  assert.equal(summary().textContent,"Snapshot-matched Top Pins content readiness: 1 ready · 1 need attention · 1 unavailable.");assert.equal(rows().length,3);assert.deepEqual(pairs(),[[pins[0].title,"Content ready"],["Bravo","Content needs attention (2)"],["Charlie","Content readiness unavailable"]]);assert.equal(harness.document.querySelectorAll(".top-pins-readiness-status").every(item=>item.tagName!=="A"),true);assert.equal(harness.document.querySelectorAll("img").length,0);assert.equal(harness.document.querySelectorAll("a").length,0);
  for(const forbidden of ["readiness-private-1","must-discard","TITLE_MISSING","private audit message","private review message","private-provider","rawAudit","issueMessage"])assert.equal(harness.hasText(forbidden),false);
  const contribution=harness.document.querySelector(".top-pins-contribution-table").textContent,before=counts();for(const selector of harness.document.querySelectorAll(".top-pins-selector"))selector.click();const chartOrder=harness.document.querySelectorAll(".top-pins-rank-label").map(item=>item.textContent);harness.document.querySelector('[data-top-pins-sort="title"]').click();harness.document.querySelector('[data-top-pins-sort="impressions"]').click();assert.deepEqual(Object.fromEntries(pairs()),Object.fromEntries([[pins[0].title,"Content ready"],["Bravo","Content needs attention (2)"],["Charlie","Content readiness unavailable"]]));assert.deepEqual(harness.document.querySelectorAll(".top-pins-rank-label").map(item=>item.textContent),chartOrder);assert.equal(harness.document.querySelector(".top-pins-contribution-table").textContent,contribution);assert.deepEqual(counts(),before);
  harness.document.querySelector('[data-pin-view="all"]').click();harness.document.querySelector('[data-pin-view="performance"]').click();await harness.settle();assert.equal(summary().textContent,"Snapshot-matched Top Pins content readiness: 1 ready · 1 need attention · 1 unavailable.");assert.deepEqual(counts(),before);await harness.clickAction("performance");assert.deepEqual(counts(),[1,1,1,1]);harness.document.querySelector('[data-pin-view="overview"]').click();assert.equal(harness.document.querySelector(".content-readiness").textContent,globalAudit);harness.document.querySelector('[data-pin-view="performance"]').click();
  await harness.clickAction("observe");assert.notEqual(summary(),undefined);assert.deepEqual(counts(),[2,1,1,1]);await harness.clickAction("observe");assert.notEqual(summary(),undefined);assert.deepEqual(counts(),[3,1,1,1]);await harness.clickAction("observe");assert.equal(table(),undefined);assert.deepEqual(counts(),[4,1,1,1]);
});
...
test("Top Pins outbound-vs-account context is exact-window, fail-closed, stable, and request-isolated",async()=>{
  const window={startDate:"2026-07-24",endDate:"2026-08-22",completedDays:30},daily=Array.from({length:30},(_,index)=>({date:new Date(Date.UTC(2026,6,24+index)).toISOString().slice(0,10),impressions:index+1,saves:0,pinClicks:0,outboundClicks:index%2})),baseValues=[{impressions:100,saves:0,pinClicks:0,outboundClicks:3},{impressions:100,saves:0,pinClicks:0,outboundClicks:0},{impressions:100,saves:0,pinClicks:0,outboundClicks:1},{impressions:300,saves:0,pinClicks:0,outboundClicks:4},{impressions:30000,saves:0,pinClicks:0,outboundClicks:400},{impressions:0,saves:0,pinClicks:0,outboundClicks:0},{impressions:null,saves:null,pinClicks:null,outboundClicks:null},{impressions:1,saves:0,pinClicks:0,outboundClicks:2}],baseOriginal=JSON.stringify(baseValues),baseTotals={impressions:1000,saves:0,pinClicks:0,outboundClicks:10};
  const open=async({values=baseValues,totals=baseTotals,accountWindow=window,topWindow=window,accountState="Available",topState="Available",accountStale=false,topStale=false,topFirst=false,verifyConnection=async()=>authenticated}={})=>{const pins=values.map((_,index)=>({pinId:`outbound-private-${index}`,title:String.fromCharCode(65+index),boardName:`Board ${index+1}`,thumbnail:null,providerPayload:"observation-private",boardId:"board-private"})),account={ok:true,state:accountState,window:accountWindow,latestAvailableDate:accountWindow?.endDate??null,totals,daily,stale:accountStale,providerPayload:"account-private",token:"account-token"},top={ok:true,state:topState,window:topWindow,sortBy:"OUTBOUND_CLICK",pins:values.map((value,index)=>({pinId:`outbound-private-${index}`,...value,providerPayload:"top-private",url:"https://provider.invalid/private"})),stale:topStale},h=await createPinterestDomHarness(preloadFor({connectionStatus:async()=>authenticated,verifyConnection,readObservation:async()=>({ok:true,state:"Completed",pins}),readAccountPerformance:async()=>account,readTopPins:async()=>top,readPerformance:async()=>({ok:true,state:"Available",window,totals:baseTotals,pins:top.pins,stale:false})})).start();h.document.querySelector('[data-pin-view="performance"]').click();assert.deepEqual([h.callCount("readAccountPerformance"),h.callCount("readTopPins"),h.callCount("readPerformance"),h.callCount("readObservation")],[0,0,0,0]);await h.clickAction("observe");if(topFirst){await h.clickAction("top-pins");assert.deepEqual([h.callCount("readAccountPerformance"),h.callCount("readTopPins")],[0,1]);assert.equal(h.hasText("Same-period account outbound click rate unavailable."),true);await h.clickAction("account-performance");assert.deepEqual([h.callCount("readAccountPerformance"),h.callCount("readTopPins")],[1,1])}else{await h.clickAction("account-performance");assert.deepEqual([h.callCount("readAccountPerformance"),h.callCount("readTopPins")],[1,0]);await h.clickAction("top-pins");assert.deepEqual([h.callCount("readAccountPerformance"),h.callCount("readTopPins")],[1,1])}return h};
  const harness=await open(),counts=()=>[harness.callCount("readAccountPerformance"),harness.callCount("readTopPins"),harness.callCount("readPerformance"),harness.callCount("readObservation")],table=()=>harness.document.querySelector(".top-pins-table"),rows=()=>table().children[1].children,headers=()=>table().children[0].children[0].children,names=()=>rows().map(row=>row.children[1].children[0].textContent),differences=()=>rows().map(row=>row.children[10].textContent),sort=()=>harness.document.querySelector('[data-top-pins-sort="outboundVsAccount"]');
  assert.deepEqual(counts(),[1,1,0,1]);const headingLabels=headers().map(cell=>cell.children[0].children[0].textContent);assert.deepEqual(headingLabels,["Rank","Pin","Board","Impressions","Saves","Pin clicks","Outbound clicks","Save rate","Pin click rate","Outbound click rate","Outbound vs account"]);assert.equal(headingLabels.indexOf("Outbound vs account"),headingLabels.indexOf("Outbound click rate")+1);assert.deepEqual(names(),["A","B","C","D","E","F","G","H"]);assert.deepEqual(differences(),["+2.00 pp","−1.00 pp","0.00 pp","+0.33 pp","+0.33 pp","—","—","+199.00 pp"]);assert.equal(differences().some(value=>/[eE][+-]?\d/.test(value)),false);const context=harness.document.querySelector(".top-pins-outbound-context");assert.equal(context.textContent,"Same-period account outbound click rate: 1.00%. Differences are percentage points and descriptive only.");assert.equal(context.getAttribute("aria-label"),"Same-period account outbound click rate: 1.00%. Differences are percentage points and descriptive only. Shared 30-day UTC period: 24.07.26 – 22.08.26.");assert.equal(sort().type,"button");assert.equal(headers()[10].getAttribute("aria-sort"),"none");
  const chartOrder=harness.document.querySelectorAll(".top-pins-rank-label").map(label=>label.textContent),contribution=()=>harness.document.querySelector(".top-pins-contribution-table").children[1].children.map(row=>row.children.map(cell=>cell.textContent)),contributionBefore=contribution();harness.document.querySelector('[data-account-sort="impressions"]').click();const accountOrder=harness.document.querySelector(".account-performance-table").children[1].children.map(row=>row.children[0].textContent),beforeLocal=counts();sort().pressKey("Enter");assert.deepEqual(names(),["H","A","D","E","C","B","F","G"]);assert.deepEqual(differences(),["+199.00 pp","+2.00 pp","+0.33 pp","+0.33 pp","0.00 pp","−1.00 pp","—","—"]);assert.equal(headers()[10].getAttribute("aria-sort"),"descending");assert.equal(headers()[10].textContent,"Outbound vs account▼");assert.deepEqual(names().slice(2,4),["D","E"]);sort().pressKey(" ");assert.deepEqual(names(),["B","C","D","E","A","H","F","G"]);assert.equal(headers()[10].getAttribute("aria-sort"),"ascending");assert.equal(headers()[10].textContent,"Outbound vs account▲");assert.deepEqual(names().slice(2,4),["D","E"]);assert.deepEqual(harness.document.querySelectorAll(".top-pins-rank-label").map(label=>label.textContent),chartOrder);assert.deepEqual(contribution(),contributionBefore);assert.deepEqual(harness.document.querySelector(".account-performance-table").children[1].children.map(row=>row.children[0].textContent),accountOrder);for(const action of ['[data-top-pins-sort="impressions"]','[data-top-pins-metric="saveRate"]','[data-account-trend-metric="saves"]'])harness.document.querySelector(action).click();assert.deepEqual(counts(),beforeLocal);harness.document.querySelector('[data-pin-view="all"]').click();harness.document.querySelector('[data-pin-view="performance"]').click();await harness.settle();assert.deepEqual(counts(),beforeLocal);await harness.clickAction("performance");assert.deepEqual(counts(),[1,1,1,1]);
  assert.equal(JSON.stringify(baseValues),baseOriginal);const topFirst=await open({topFirst:true});assert.equal(topFirst.hasText("Same-period account outbound click rate: 1.00%. Differences are percentage points and descriptive only."),true);assert.deepEqual([topFirst.callCount("readAccountPerformance"),topFirst.callCount("readTopPins"),topFirst.callCount("readObservation")],[1,1,1]);const zeroAccount=await open({totals:{...baseTotals,outboundClicks:0},values:[{impressions:100,saves:0,pinClicks:0,outboundClicks:0},{impressions:100,saves:0,pinClicks:0,outboundClicks:2}]});assert.equal(zeroAccount.hasText("Same-period account outbound click rate: 0.00%. Differences are percentage points and descriptive only."),true);assert.deepEqual(zeroAccount.document.querySelector(".top-pins-table").children[1].children.map(row=>row.children[10].textContent),["0.00 pp","+2.00 pp"]);const unrounded=await open({totals:{...baseTotals,impressions:3,outboundClicks:1},values:[{impressions:3,saves:0,pinClicks:0,outboundClicks:2}]});assert.equal(unrounded.document.querySelector(".top-pins-table").children[1].children[0].children[10].textContent,"+33.33 pp");
  for(const [label,accountWindow,topWindow] of [["start",{startDate:"2026-07-23",endDate:"2026-08-21",completedDays:30},window],["end",{startDate:"2026-07-25",endDate:"2026-08-23",completedDays:30},window],["days",{...window,completedDays:29},window]]){const mismatch=await open({accountWindow,topWindow});assert.equal(mismatch.hasText("Same-period account outbound click rate unavailable."),true,label);assert.equal(mismatch.document.querySelector(".top-pins-table").children[1].children.every(row=>row.children[10].textContent==="—"),true,label)}
  for(const invalid of [null,undefined,"10",1.5,-1,Infinity,Number.MAX_SAFE_INTEGER+1]){for(const totals of [{...baseTotals,impressions:invalid},{...baseTotals,outboundClicks:invalid}]){const unavailable=await open({totals});assert.equal(unavailable.hasText("Same-period account outbound click rate unavailable."),true);assert.equal(unavailable.document.querySelector(".top-pins-table").children[1].children.every(row=>row.children[10].textContent==="—"),true)}}const zeroDenominator=await open({totals:{...baseTotals,impressions:0}});assert.equal(zeroDenominator.hasText("Same-period account outbound click rate unavailable."),true);
  const invalidValues=[null,undefined,"4",4.5,-1,Infinity,Number.MAX_SAFE_INTEGER+1],invalidRows=invalidValues.flatMap(value=>[{impressions:value,saves:0,pinClicks:0,outboundClicks:1},{impressions:100,saves:0,pinClicks:0,outboundClicks:value}]),rowInvalid=await open({values:invalidRows});assert.equal(rowInvalid.document.querySelector(".top-pins-table").children[1].children.every(row=>row.children[10].textContent==="—"),true);const bounded=await open({totals:{...baseTotals,impressions:10000},values:Array.from({length:26},(_,index)=>({impressions:100,saves:0,pinClicks:0,outboundClicks:index}))});assert.equal(bounded.document.querySelector(".top-pins-table").children[1].children.length,25);
  const stale=await open({accountState:"Failed",topState:"Failed",accountStale:true,topStale:true});assert.equal(stale.hasText("Retained same-period account context; data may be stale."),true);assert.equal(stale.hasText("Same-period account outbound click rate: 1.00%. Differences are percentage points and descriptive only."),true);
  const shifted={startDate:"2026-07-23",endDate:"2026-08-21",completedDays:30},dynamicPins=baseValues.map((_,index)=>({pinId:`dynamic-${index}`,title:`Dynamic ${index}`,boardName:"Board",thumbnail:null})),dynamicAccount={ok:true,state:"Available",window,latestAvailableDate:window.endDate,totals:baseTotals,daily,stale:false},dynamicTop={ok:true,state:"Available",window,sortBy:"OUTBOUND_CLICK",pins:baseValues.map((value,index)=>({pinId:`dynamic-${index}`,...value})),stale:false},dynamic=await createPinterestDomHarness(preloadFor({connectionStatus:async()=>authenticated,readObservation:sequence({ok:true,state:"Completed",pins:dynamicPins},{ok:true,state:"CompletedWithWarnings",pins:dynamicPins},{ok:true,state:"Completed",pins:[{pinId:"changed",title:"Changed",boardName:"Board",thumbnail:null}]}),readAccountPerformance:sequence(dynamicAccount,{...dynamicAccount,window:shifted,latestAvailableDate:shifted.endDate},dynamicAccount),readTopPins:sequence(dynamicTop,{...dynamicTop,window:shifted},dynamicTop)})).start();await dynamic.clickAction("observe");dynamic.document.querySelector('[data-pin-view="performance"]').click();await dynamic.clickAction("account-performance");await dynamic.clickAction("top-pins");assert.equal(dynamic.hasText("Same-period account outbound click rate: 1.00%. Differences are percentage points and descriptive only."),true);await dynamic.clickAction("account-performance");assert.equal(dynamic.hasText("Same-period account outbound click rate unavailable."),true);await dynamic.clickAction("account-performance");assert.equal(dynamic.hasText("Same-period account outbound click rate: 1.00%. Differences are percentage points and descriptive only."),true);await dynamic.clickAction("top-pins");assert.equal(dynamic.hasText("Same-period account outbound click rate unavailable."),true);await dynamic.clickAction("top-pins");assert.equal(dynamic.hasText("Same-period account outbound click rate: 1.00%. Differences are percentage points and descriptive only."),true);const dynamicBefore=[dynamic.callCount("readAccountPerformance"),dynamic.callCount("readTopPins"),dynamic.callCount("readObservation")];await dynamic.clickAction("observe");assert.notEqual(dynamic.document.querySelector(".top-pins-table"),undefined);assert.deepEqual([dynamic.callCount("readAccountPerformance"),dynamic.callCount("readTopPins"),dynamic.callCount("readObservation")],[dynamicBefore[0],dynamicBefore[1],dynamicBefore[2]+1]);await dynamic.clickAction("observe");assert.equal(dynamic.document.querySelector(".top-pins-table"),undefined);
  const auth=await open({verifyConnection:async()=>({ok:true,state:"ReauthorizationRequired"})});await auth.clickAction("verify");assert.equal(auth.document.querySelector(".top-pins-table"),undefined);for(const forbidden of ["outbound-private-0","board-private","observation-private","account-private","account-token","top-private","provider.invalid"])assert.equal(harness.hasText(forbidden)||topFirst.hasText(forbidden)||stale.hasText(forbidden)||auth.hasText(forbidden),false);assert.equal(harness.document.querySelectorAll("a").length,0);
});
...
test("observed rates enforce exact local formulas, unrounded sorting, uncapped values, and safe missing states",async()=>{
  ...
  assert.deepEqual(rows().map(row=>row.children.slice(7,10).map(cell=>cell.textContent)),[["33.33%","0.00%","—"],["33.33%","0.00%","120.00%"],["—","—","—"],["—","—","—"]]);saveSort.pressKey("Enter");assert.deepEqual(rows().slice(0,2).map(row=>row.children[1].children[0].textContent),["Rate 2","Rate 1"]);assert.equal(JSON.stringify(values),original);assert.equal(harness.callCount("readTopPins"),1);
