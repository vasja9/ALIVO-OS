...
test("Top Pins comparison and table sorting are local, stable, accessible, immutable, and raw-ID safe",async()=>{
  ...
  const requestCounts=()=>["readTopPins","readAccountPerformance","readPerformance","readObservation"].map(name=>harness.callCount(name)),selectors=()=>harness.document.querySelectorAll(".top-pins-selector"),chart=()=>harness.document.querySelector(".top-pins-chart"),bars=()=>harness.document.querySelectorAll(".top-pins-bar"),table=harness.document.querySelector(".top-pins-table"),rows=()=>table.children[1].children,headers=()=>table.children[0].children[0].children,button=key=>harness.document.querySelector(`[data-top-pins-sort="${key}"]`);
  assert.deepEqual(selectors().map(item=>item.textContent),["Impressions","Saves","Pin clicks","Outbound clicks"]);assert.deepEqual(selectors().map(item=>item.getAttribute("aria-pressed")),["false","false","false","true"]);assert.equal(chart().getAttribute("viewBox"),"0 0 640 260");assert.equal(chart().getAttribute("role"),"img");assert.equal(chart().getAttribute("aria-label"),"Outbound clicks Top Pins comparison from 24.07.26 to 22.08.26 with 3 usable values.");assert.deepEqual(bars().map(bar=>bar.getAttribute("aria-label")),["Rank 2: alpha; Outbound clicks: 8","Rank 3: Alpha; Outbound clicks: 8","Rank 1: Zulu; Outbound clicks: 3"]);assert.equal(Number(bars()[0].getAttribute("height"))>Number(bars()[2].getAttribute("height")),true);
  assert.deepEqual(rows().map(row=>row.children[1].textContent),["Zulu","alpha","Alpha","Missing"]);assert.deepEqual(headers().map(cell=>cell.getAttribute("aria-sort")),["ascending","none","none","none","none","none","none"]);assert.equal(headers()[0].textContent,"Rank▲");const before=requestCounts();
  for(const selector of selectors()){selector.pressKey(selector===selectors()[0]?"Enter":" ");assert.equal(selectors().filter(item=>item.getAttribute("aria-pressed")==="true").length,1)}assert.deepEqual(requestCounts(),before);assert.deepEqual(rows().map(row=>row.children[1].textContent),["Zulu","alpha","Alpha","Missing"]);
  const chartOrder=bars().map(bar=>bar.getAttribute("aria-label"));for(const [key,firstDirection] of [["rank","descending"],["title","ascending"],["boardName","ascending"],["impressions","descending"],["saves","descending"],["pinClicks","descending"],["outboundClicks","descending"]]){button(key).pressKey(key==="rank"?"Enter":" ");const active=headers().find(cell=>cell.children[0].dataset.topPinsSort===key);assert.equal(active.getAttribute("aria-sort"),firstDirection);assert.equal(/[▲▼]$/.test(active.textContent),true);assert.equal(headers().filter(cell=>cell.getAttribute("aria-sort")!=="none").length,1);if(["impressions","saves","pinClicks","outboundClicks"].includes(key))assert.equal(rows().at(-1).children[1].textContent,"Missing");button(key).click();if(["impressions","saves","pinClicks","outboundClicks"].includes(key))assert.equal(rows().at(-1).children[1].textContent,"Missing")}
...
test("Top Pins chart safely handles all-zero, one-value, zero-usable, and maximum-25 inputs",async()=>{
  ...
  const zero=await open([0,0,0]);assert.equal(zero.document.querySelectorAll(".top-pins-bar").length,3);assert.equal(zero.document.querySelectorAll(".top-pins-bar").every(bar=>bar.getAttribute("height")==="0"),true);
  const one=await open([null,7,null]);const oneBar=one.document.querySelector(".top-pins-bar");assert.equal(one.document.querySelectorAll(".top-pins-bar").length,1);assert.equal(Number(oneBar.getAttribute("x"))>200&&Number(oneBar.getAttribute("x"))<400,true);
  const missing=await open([null,null]);assert.equal(missing.hasText("No usable Top Pins values"),true);assert.equal(missing.document.querySelector(".top-pins-chart"),undefined);
  const bounded=await open(Array.from({length:26},(_,index)=>index));assert.equal(bounded.document.querySelectorAll(".top-pins-bar").length,25);assert.equal(bounded.document.querySelector(".top-pins-table").children[1].children.length,25);
});
