...
test("live composition routes verification and observation through the governed adapter and workflow", async () => {
  ...
  assert.deepEqual(topPins.pins[0],{title:"Observed pin 1",boardName:"<b>Main board</b>",impressions:20,saves:2,pinClicks:3,outboundClicks:4,lifecycle:{createdAt:null,completedAgeDays:null,cohort:"Unknown",outboundState:"ReachedAlivoEu",coverage:"Observed30CompletedUtcDays"},contentReadiness:{status:"NeedsAttention",issueCount:2,requiredIssueCount:1,reviewIssueCount:1},contentReadinessDetails:{required:["Review the destination: it is outside alivo.eu."],review:["Add a Pin description for Pinterest relevance."]}});
  assert.equal(Object.isFrozen(topPins.pins[0]),true);assert.equal(Object.isFrozen(topPins.pins[0].lifecycle),true);assert.equal(Object.isFrozen(topPins.pins[0].contentReadiness),true);assert.equal(Object.isFrozen(topPins.pins[0].contentReadinessDetails),true);assert.equal(Object.isFrozen(topPins.pins[0].contentReadinessDetails?.required),true);assert.equal(Object.isFrozen(topPins.pins[0].contentReadinessDetails?.review),true);
  assert.equal(/pin-0|pinId|provider|issue_codes|message|code|url|base64|thumbnail|oauth|token|media/i.test(JSON.stringify(topPins)),false);
...
test("trusted Top Pins composition joins fixed readiness details by internal Pin ID and discards IDs and codes",()=>{
  ...
  assert.deepEqual(joined.pins,[
    {title:"Bravo",boardName:"Board B",impressions:20,saves:2,pinClicks:3,outboundClicks:4,lifecycle:{createdAt:null,completedAgeDays:null,cohort:"Unknown",outboundState:"ReachedAlivoEu",coverage:"Observed30CompletedUtcDays"},contentReadiness:{status:"NeedsAttention",issueCount:12,requiredIssueCount:4,reviewIssueCount:8},contentReadinessDetails:{required,review}},
    {title:"Alpha",boardName:"Board A",impressions:10,saves:1,pinClicks:2,outboundClicks:3,lifecycle:{createdAt:null,completedAgeDays:null,cohort:"Unknown",outboundState:"ReachedAlivoEu",coverage:"Observed30CompletedUtcDays"},contentReadiness:{status:"Ready",issueCount:0,requiredIssueCount:0,reviewIssueCount:0},contentReadinessDetails:{required:[],review:[]}},
  ]);
  assert.equal(Object.isFrozen(joined),true);assert.equal(Object.isFrozen(joined.pins),true);assert.equal(joined.pins.every(pin=>Object.isFrozen(pin)&&Object.isFrozen(pin.lifecycle)&&Object.isFrozen(pin.contentReadiness)&&Object.isFrozen(pin.contentReadinessDetails)&&Object.isFrozen(pin.contentReadinessDetails?.required)&&Object.isFrozen(pin.contentReadinessDetails?.review)),true);
